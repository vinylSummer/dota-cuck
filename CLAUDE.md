...
command over the worker stream → worker connects its warm session if needed (relogin via
`login_key` when available), lists friends, replies `FriendsResult` correlated by `request_id`
...
user_login_password
  ...
  ─AES-256-GCM→ enc_password + enc_nonce  (stored in DB)
```
