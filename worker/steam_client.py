...
class SteamSession:
    ...
    def resolve_match_id(self, target_steam_id: str, refresh_token: str) -> int | None:
        """Resolve the target friend's live watchable match id from rich presence.

        Logs the warm session on with the refresh token if needed (zero Steam
        Guard), then requests the target's persona state with the RichPresence
        flag and polls for ``WatchableGameID``. Returns ``None`` if the target is
        not in a live, watchable, public match. Runs on the warm python-steam
        ...
        client = self._ensure_client()
        self.login_with_token(refresh_token)
